...
async fn main() -> std::io::Result<()> {
    ...
    let formatting_layer = BunyanFormattingLayer::new("gdsc_rust_study".into(), std::io::stdout);
    let subscriber = Registry::default().with(env_filter).with(JasonStorageLayer).with(formatting_layer);
